defmodule PinServer.SimulatorInputAdapter do
  alias PinServer.Pin

  def start(pin_server, %Pin{pin_number: pin_number}, direction, level \\ :low) do
    {:ok, pin} = PinServer.PinServer.update_pin_status(pin_number, %{direction: direction, level: level})
    ...
    receive do
      ...
      {:change_level, new_level} ->
        PinServer.PinServer.update_pin_status(pin_number, %{level: new_level})
